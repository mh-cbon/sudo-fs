...
var platformNodebinSpawn = function (bin, args, opts) {
  if (process.platform.match(/win/)) {
    return spawn('node_modules\\.bin\\' + bin + '.cmd', args, opts);
  }
  args.unshift('node_modules/.bin/' + bin)
  return spawn(process.argv[0], args, opts);
}

var createReadStream = function (path) {
  var sPath = path.toString();
  var child = platformNodebinSpawn('catf', [sPath], {stdio: ['ignore', 'pipe', 'pipe']});
...
var createWriteStream = function (path, options) {
  ...
  var child = platformNodebinSpawn('fwrite', [sPath], {stdio: 'pipe'});
...
var mkdir = function (fPath, mod, callback) {
  ...
  var child = platformNodebinSpawn('mkdirp', args, {stdio: 'pipe'});
...
var unlink = function (fPath, callback) {
  ...
  var child = platformNodebinSpawn('rimraf', [fPath], {stdio: 'pipe'});
...
var touch = function (fPath, options, callback) {
  ...
  var child = platformNodebinSpawn('touch', args, {stdio: 'pipe'});
